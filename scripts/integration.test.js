...
test("final-schema authentication, isolation, stock, order, delivery, points, membership, payment and WhatsApp controls",async t=>{
  ...
  const dispatched=await request(`/orders/${created.body.id}/transit`,{cookie:rider.cookie,method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({status:"In Transit"})});assert.equal(dispatched.response.status,200);assert.equal(dispatched.body.statusCode,"DISPATCHED");
  const delivered=await request(`/orders/${created.body.id}/transit`,{cookie:rider.cookie,method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({status:"DELIVERED",codStatus:"COLLECTED_BY_RIDER"})});assert.equal(delivered.response.status,200);assert.equal(delivered.body.statusCode,"DELIVERED");assert.equal(delivered.body.codStatus,"COLLECTED_BY_RIDER");assert.equal(delivered.body.paymentStatus,"PAID");
  ...
  assert.equal((await request(`/orders/${created.body.id}/settlement`,{cookie:rider.cookie,method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({codStatus:"CLEARED_TO_ADMIN"})})).response.status,403);
  const settlement=await request(`/orders/${created.body.id}/settlement`,{cookie:admin.cookie,method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({codStatus:"CLEARED_TO_ADMIN",codClearanceNote:"Integration settlement"})});assert.equal(settlement.response.status,200);assert.equal(settlement.body.codStatus,"CLEARED_TO_ADMIN");assert.equal(settlement.body.paymentStatus,"PAID");

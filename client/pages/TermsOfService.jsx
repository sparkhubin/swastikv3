...
export default function TermsOfService() {
  ...
  const { termsSections } = useData();

  const isHindi = language === 'hi';

  return (
    <div className="px-4 md:px-8 max-w-4xl mx-auto py-8 text-white min-h-[60vh]" id="terms-of-service-view">
      ...
      <div className="bg-white/5 backdrop-blur-xl border border-white/12 p-6 rounded-2xl shadow-xl flex items-center gap-4 mb-8">
        <div className="w-12 h-12 bg-cyan-500/20 text-cyan-300 border border-cyan-400/30 rounded-xl flex items-center justify-center shrink-0">
          <Gavel className="h-6 w-6 stroke-[2]" />
        </div>
        <div>
      ...
      <div className="mt-8 bg-yellow-500/10 border border-yellow-500/25 p-4 rounded-xl flex items-start gap-3 animate-pulse">
        <AlertTriangle className="h-5 w-5 text-yellow-300 shrink-0 mt-0.5" />
        <p className="text-slate-400 text-[11px] leading-relaxed">
          {isHindi 
            ? "सेवा की शर्तों का उल्लंघन करने पर खाता तत्काल निलंबित किया जा सकता है।"
            : "Violation of any terms listed here may result in suspension or termination of your active service account immediately."
          }
        </p>
      </div>
    </div>

...
export default function CartTab({ isHindi }) {
  ...
  return (
    <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-xl text-white space-y-6">
      <div className="border-b border-white/10 pb-4 flex items-center justify-between">
        <div>
          <h3 className="font-bold text-base text-white uppercase tracking-wider flex items-center gap-2 text-glow">
            ...
            <span className="px-2.5 py-0.5 rounded-full bg-emerald-400/20 text-emerald-300 text-xs font-mono font-bold">
      ...
      {cartItems.length === 0 ? (
        <div className="py-16 flex flex-col items-center justify-center text-center space-y-4 bg-slate-900/40 rounded-2xl border border-white/5">
          <div className="w-16 h-16 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400">
            <ShoppingBag className="w-8 h-8" />
          </div>
          <div className="space-y-1">
            <h4 className="text-sm font-bold text-white">{isHindi ? "आपकी कार्ट खाली है!" : "Your Cart is Currently Empty"}</h4>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 space-y-3">
            {cartItems.map((item, idx) => {
              ...
              return (
                <div 
                  key={idx}
                  className="bg-slate-900/70 border border-white/10 p-4 rounded-xl flex items-center gap-4 hover:border-white/20 transition-all"
                >
                  <img 
                  ...
                  <div className="flex-1 min-w-0">
                    <h5 className="font-bold text-xs text-white truncate">{isHindi ? (p.nameHi || p.nameEn || p.name) : (p.nameEn || p.name)}</h5>
                    <p className="text-[10px] text-slate-400 font-mono mt-0.5">Unit: {unit}</p>
                    <p className="text-xs font-bold text-cyan-300 font-mono mt-1">₹{p.price || 100}</p>
                  </div>

                  {/* Quantity Modifier */}
                  <div className="flex items-center gap-2 bg-slate-950 border border-white/10 px-2.5 py-1 rounded-lg font-mono">
                    <button
                      type="button"
                  ...
                  {/* Line Total */}
                  <div className="text-right font-mono font-black text-sm text-white min-w-[60px]">
                  ...
                  <button
                    ...
                    className="text-slate-500 hover:text-rose-400 transition-all p-1.5 cursor-pointer"
                    title="Remove Item"
                  >
          ...
          {/* Cart Summary Card */}
          <div className="lg:col-span-4 bg-slate-900/90 border border-white/10 p-5 rounded-2xl flex flex-col justify-between h-fit space-y-4 font-mono">
            <div>
              <h4 className="text-xs font-extrabold text-white uppercase tracking-wider border-b border-white/10 pb-3 mb-4">

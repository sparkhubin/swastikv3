...
export default function OrdersTab({
  ...
  return (
    <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-4 sm:p-6 shadow-xl text-white">
        ...
        <>
          ...
          <div className="block md:hidden">
            <div className="flex items-center justify-between mb-3 text-xs">
              ...
              <div className="flex items-center gap-1 bg-slate-900/90 p-1 rounded-xl border border-white/15 select-none">
                <button
                  type="button"
            ...
            {mobileViewMode === 'grid' ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {displayedMobileOrders.map(o => (
                  <div 
                    key={o.id} 
                    className="bg-slate-900/80 border border-white/10 rounded-xl p-3.5 flex flex-col justify-between space-y-2.5 hover:border-cyan-500/40 transition-all shadow-md"
                  >
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        <span className="font-mono font-bold text-cyan-300 text-xs block">#{o.id}</span>
                        <p className="text-[10px] text-slate-400 mt-0.5">{o.date || o.createdAt}</p>
                      </div>
                    ...
                    <div className="text-[11px] text-slate-300 line-clamp-1 border-t border-white/5 pt-2">
                      {(o.items || []).map(i => i.nameEn || i.nameHi || i.name).join(', ')}
                    </div>

                    <div className="flex justify-between items-center border-t border-white/5 pt-2">
                      <div className="flex flex-col">
                        <span className="text-[9px] text-slate-500 uppercase">{o.paymentMethod || 'UPI'}</span>
                        <span className="font-black text-white text-sm font-mono">₹{o.totalAmount || o.grandTotal || o.total}</span>
                      </div>
                      <button
                        type="button"
                        onClick={() => setSelectedOrder(o)}
                        className="px-3 py-1.5 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 border border-cyan-400/30 rounded-lg text-[10px] font-bold flex items-center gap-1 active:scale-95 transition-all"
                      >
              ...
              <div className="space-y-2">
                {displayedMobileOrders.map(o => (
                  <div 
                    key={o.id} 
                    className="bg-slate-900/80 border border-white/10 rounded-xl p-3 flex items-center justify-between gap-3 hover:border-cyan-500/30 transition-all"
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-cyan-300 text-xs truncate">#{o.id}</span>
                      ...
                      <p className="text-[10px] text-slate-400 font-mono mt-0.5 truncate">{o.date || o.createdAt}</p>
                    </div>

                    <div className="flex items-center gap-2.5 shrink-0">
                      <span className="font-black text-white text-xs font-mono">₹{o.totalAmount || o.grandTotal || o.total}</span>
                      <button
                        type="button"
                        onClick={() => setSelectedOrder(o)}
                        className="p-1.5 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 border border-cyan-400/30 rounded-lg text-xs"
                      >
            ...
            {filteredOrders.length > mobileLimit && (
              <button
                ...
                className="mt-4 w-full py-2.5 bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/25 text-cyan-300 font-bold text-xs uppercase tracking-wider rounded-xl transition-all active:scale-98 text-center"
              >
